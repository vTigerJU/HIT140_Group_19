...
#pd.set_option('display.width', 1000)        # Optional: To control the width of the display
...
def linRegResult(x, y, test_size):
    ...
    for feature, coef_val in zip(x.columns, model.coef_.flatten()):
...
def totalScreenTime(watchTime):
    ...
    # New features: total weekday time and total weekend time
    watchTime['totalWeekdayTime'] = watchTime['C_wk'] + watchTime['G_wk'] + watchTime['S_wk'] + watchTime['T_wk']
    watchTime['totalWeekendTime'] = watchTime['C_we'] + watchTime['G_we'] + watchTime['S_we'] + watchTime['T_we']
...
# Adding interaction terms and new features
df['smartphone_gender'] = df['smartphoneTime'] * df['gender']
df['totalTime_deprived'] = (df['computerTime'] + df['gameTime'] + df['smartphoneTime'] + df['tvTime']) * df['deprived']

x = df[['computerTime', 'gameTime', 'smartphoneTime', 'tvTime', 'totalWeekdayTime', 'totalWeekendTime', 
        'smartphone_gender', 'totalTime_deprived', 'gender', 'deprived', 'minority']]  # New features + interactions
...
correlation_matrix = df[['computerTime', 'gameTime', 'smartphoneTime', 'tvTime', 'totalWeekdayTime', 'totalWeekendTime', 'smartphone_gender', 'totalTime_deprived']].corr()
