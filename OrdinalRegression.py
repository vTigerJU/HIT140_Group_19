...
# Assume df is your DataFrame with features and the 'compositeScore' target
x = df[['computerTime', 'gameTime', 'smartphoneTime', 'tvTime', 'gender', 'deprived', 'minority']]
y = df['compositeScore']  # Replace with your ordered target variable (e.g., low, medium, high)

# Split the data into training and test sets
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=0)

# Using the Ordinal Logistic Regression (mord library)
model = m.LogisticIT()  # Logistic Ordinal Regression
model.fit(x_train, y_train)
y_pred = model.predict(x_test)
